...
public interface VideoRecommendContract {
    ...
    interface View extends IView {
        //        最热栏目
        void getViewHotColumn(List<VideoHotColumn> mHomeHotColumn);
        void getViewCarousel(List<VideoCarousel> mHomeCarousels);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {
        Observable<List<VideoHotColumn>> getModelHotColumn(@QueryMap Map<String, String> maps);
        Observable<List<VideoCarousel>> getModelHomeCarousel(@QueryMap Map<String, String> maps);
    }
}